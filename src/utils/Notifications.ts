...
function _processNotification(notification) {
  ...
  if (Platform.OS === "ios") {
    ...
    if (notification.ios) {
      ...
      notification.data._displayInForeground =
        notification.ios._displayInForeground;
...
let IS_USING_NEW_BINARY =
  typeof ExponentNotifications.createChannel === "function";

async function _legacyReadChannel(id: string): Promise<Channel | null> {
  try {
    let channelString = await AsyncStorage.getItem(
      `${ASYNC_STORAGE_PREFIX}${id}`
    );
    if (channelString) {
    ...
    try {
      ...
      if (keys && keys.length) {
        let filteredKeys = keys.filter(
          key => !key.startsWith(ASYNC_STORAGE_PREFIX)
        );
...
export default {
  ...
  createChannelAndroidAsync(id: string, channel: Channel): Promise<void> {
    if (Platform.OS !== "android") {
      console.warn(
        `createChannelAndroidAsync(...) has no effect on ${Platform.OS}`
      );
  ...
  deleteChannelAndroidAsync(id: string): Promise<void> {
    if (Platform.OS !== "android") {
      console.warn(
        `deleteChannelAndroidAsync(...) has no effect on ${Platform.OS}`
      );
      return Promise.resolve();
    }
    // This codepath will never be triggered in SDK 28 and above
  ...
  async presentLocalNotificationAsync(
    ...
    if (Platform.OS !== "android") {
      return await ExponentNotifications.presentLocalNotification(
        nativeNotification
      );
    } else {
      let _channel;
      ...
      if (IS_USING_NEW_BINARY) {
        ...
        return ExponentNotifications.presentLocalNotification(
          nativeNotification
        );
      }
    }
  ...
  cancelScheduledNotificationAsync(
    ...
    if (Platform.OS === "android" && typeof notificationId === "string") {
      return ExponentNotifications.cancelScheduledNotificationWithStringIdAsync(
        notificationId
      );
    }
    return ExponentNotifications.cancelScheduledNotificationAsync(
      notificationId
    );
  },
  ...
  async scheduleNotificationWithCalendarAsync(
    ...
    return ExponentNotifications.scheduleNotificationWithCalendar(
      nativeNotification,
      options
    );
  },
  ...
  async scheduleNotificationWithTimerAsync(
    ...
    return ExponentNotifications.scheduleNotificationWithTimer(
      nativeNotification,
      options
    );
  }
